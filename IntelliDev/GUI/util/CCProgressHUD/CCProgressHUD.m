...
@property (nonatomic, readwrite) CCProgressHUDMaskType maskType;
...
- (void)showWithStatus:(NSString*)string maskType:(CCProgressHUDMaskType)hudMaskType networkIndicator:(BOOL)show;
...
static CCProgressHUD *sharedView = nil;
...
+ (CCProgressHUD*)sharedView {
	
	if(sharedView == nil)
		sharedView = [[CCProgressHUD alloc] initWithFrame:[[UIScreen mainScreen] bounds]];
...
+ (void)setStatus:(NSString *)string {
	[[CCProgressHUD sharedView] setStatus:string];
}

#pragma mark - Show Methods

+ (void)show {
	[CCProgressHUD showWithStatus:nil networkIndicator:SVProgressHUDShowNetworkIndicator];
}

+ (void)showWithStatus:(NSString *)status {
    [CCProgressHUD showWithStatus:status networkIndicator:SVProgressHUDShowNetworkIndicator];
}

+ (void)showWithMaskType:(CCProgressHUDMaskType)maskType {
    [CCProgressHUD showWithStatus:nil maskType:maskType networkIndicator:SVProgressHUDShowNetworkIndicator];
}

+ (void)showWithStatus:(NSString*)status maskType:(CCProgressHUDMaskType)maskType {
    [CCProgressHUD showWithStatus:status maskType:maskType networkIndicator:SVProgressHUDShowNetworkIndicator];
}

+ (void)showWithStatus:(NSString *)status networkIndicator:(BOOL)show {
    [CCProgressHUD showWithStatus:status maskType:CCProgressHUDMaskTypeNone networkIndicator:show];
}

+ (void)showWithMaskType:(CCProgressHUDMaskType)maskType networkIndicator:(BOOL)show {
    [CCProgressHUD showWithStatus:nil maskType:maskType networkIndicator:show];
}

+ (void)showWithStatus:(NSString*)status maskType:(CCProgressHUDMaskType)maskType networkIndicator:(BOOL)show {
    [[CCProgressHUD sharedView] showWithStatus:status maskType:maskType networkIndicator:show];
}

+ (void)showSuccessWithStatus:(NSString *)string {
    [CCProgressHUD show];
    [CCProgressHUD dismissWithSuccess:string afterDelay:1];
}
...
+ (void)showInView:(UIView*)view {
    [CCProgressHUD showWithStatus:nil maskType:CCProgressHUDMaskTypeNone networkIndicator:SVProgressHUDShowNetworkIndicator];
}

+ (void)showInView:(UIView*)view status:(NSString*)string {
    [CCProgressHUD showWithStatus:string maskType:CCProgressHUDMaskTypeNone networkIndicator:SVProgressHUDShowNetworkIndicator];
}

+ (void)showInView:(UIView*)view status:(NSString*)string networkIndicator:(BOOL)show {
    [CCProgressHUD showWithStatus:string maskType:CCProgressHUDMaskTypeNone networkIndicator:show];
}
...
+ (void)showInView:(UIView*)view status:(NSString*)string networkIndicator:(BOOL)show posY:(CGFloat)posY maskType:(CCProgressHUDMaskType)hudMaskType {
    [CCProgressHUD showWithStatus:string maskType:hudMaskType networkIndicator:show];
}

#pragma mark - Dismiss Methods

+ (void)dismiss {
	[[CCProgressHUD sharedView] dismiss];
}

+ (void)dismissWithSuccess:(NSString*)successString {
	[[CCProgressHUD sharedView] dismissWithStatus:successString error:NO];
}

+ (void)dismissWithSuccess:(NSString *)successString afterDelay:(NSTimeInterval)seconds {
    [[CCProgressHUD sharedView] dismissWithStatus:successString error:NO afterDelay:seconds];
}

+ (void)dismissWithError:(NSString*)errorString {
	[[CCProgressHUD sharedView] dismissWithStatus:errorString error:YES];
}

+ (void)dismissWithError:(NSString *)errorString afterDelay:(NSTimeInterval)seconds {
    [[CCProgressHUD sharedView] dismissWithStatus:errorString error:YES afterDelay:seconds];
}
...
- (void)drawRect:(CGRect)rect {
    ...
    switch (self.maskType) {
        ...
        case CCProgressHUDMaskTypeBlack: {
            [[UIColor colorWithWhite:0 alpha:0.5] set];
            CGContextFillRect(context, self.bounds);
            break;
        }
            
        case CCProgressHUDMaskTypeGradient: {
...
- (void)showWithStatus:(NSString*)string maskType:(CCProgressHUDMaskType)hudMaskType networkIndicator:(BOOL)show {
    ...
    if(self.maskType != CCProgressHUDMaskTypeNone)
    ...
    if(![self isKeyWindow]) {
        ...
        [[UIApplication sharedApplication].windows enumerateObjectsWithOptions:NSEnumerationReverse usingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
            ...
            if(window.windowLevel == UIWindowLevelNormal && ![[window class] isEqual:[CCProgressHUD class]]) {
...
- (void)dismissWithStatus:(NSString *)string error:(BOOL)error afterDelay:(NSTimeInterval)seconds {
	...
	if(error)
		self.imageView.image = [UIImage imageNamed:@"CCProgressHUD.bundle/error1.png"];
	else
		self.imageView.image = [UIImage imageNamed:@"CCProgressHUD.bundle/success1.png"];
...
- (CGFloat)visibleKeyboardHeight {
    ...
    for (UIWindow *testWindow in [[UIApplication sharedApplication] windows]) {
        if(![[testWindow class] isEqual:[UIWindow class]] && ![[testWindow class] isEqual:[CCProgressHUD class]]) {
